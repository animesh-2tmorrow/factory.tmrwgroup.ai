...
function deriveWorkdirFromRepo(repo) {
    const match = repo.match(/([^/:]+?)(?:\.git)?$/);
    return match?.[1] ?? "project";
}
function parseBootstrapConfig(workspaceRoot) {
    ...
    const workdir = process.env.AGENT_BOOTSTRAP_WORKDIR?.trim() ||
...
async function runShell(command, cwd, label) {
    await runProcess("bash", ["-lc", command], { cwd, label });
}
async function setupGitSsh(summary) {
...
async function ensureRepo(config, summary) {
    if (!config.repo) {
    ...
    if (!hasProject) {
        await runShell(`git clone --depth 1 --branch ${config.branch} ${config.repo} ${projectDir}`, config.workspaceRoot, `clone repo ${config.repo}`);
...
export async function bootstrapWorkspace(workspaceRoot) {

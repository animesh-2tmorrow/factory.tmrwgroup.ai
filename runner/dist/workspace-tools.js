...
function truncate(text, max = MAX_OUTPUT_BYTES) {
    if (text.length <= max)
        return text;
...
export const WORKSPACE_TOOL_SPECS = [
    {
        toolSpec: {
            name: "shell_command",
            description: "Execute shell commands to complete the user's task. " +
    ...
    },
    {
        toolSpec: {
            name: "read_file",
            description: "Read the contents of a file needed for the task. Prefer workspace/project files. Max 5MB.",
            inputSchema: {
                json: {
                    type: "object",
                    properties: {
    ...
    },
    {
        toolSpec: {
            name: "write_file",
            description: "Write content to a file for task execution. Creates parent directories if needed. Max 5MB.",
            inputSchema: {
                json: {
                    type: "object",
                    properties: {
    ...
    },
    {
        toolSpec: {
            name: "web_search",
            description: "Search the web and return results. Use this when the user asks a question " +
...
export async function executeShellCommand(command, workingDir) {
    ...
    return new Promise((resolve) => {
        const child = execFile("/bin/bash", ["-c", command], {
            ...
            const combinedOutput = truncate((stdout || "") + (stderr ? `\n[stderr]\n${stderr}` : ""));
            if (error) {
                resolve({
                    success: false,
...
export async function executeReadFile(path) {
...
export async function executeWriteFile(path, content) {
    if (!path || typeof path !== "string") {
        return { success: false, error: "Path is required" };
    }
...
export async function executeListDir(path) {
...
export async function executeWebFetch(url, headers) {
...
export async function executeWebSearch(query, numResults) {
...
// ─── Dispatcher ───
export async function invokeWorkspaceTool(name, args) {
    const agentId = process.env.AGENT_ID ?? "unknown";
    console.log(`[runner] Tool call: ${name} args=${JSON.stringify(args).slice(0, 200)} agent=${agentId}`);
    switch (name) {
        case "shell_command":
            return executeShellCommand(args.command, args.working_dir);
        case "read_file":
            return executeReadFile(args.path);
        case "write_file":
            return executeWriteFile(args.path, args.content);
        case "list_dir":
            return executeListDir(args.path);
        case "web_fetch":
            return executeWebFetch(args.url, args.headers);
        case "web_search":
            return executeWebSearch(args.query, args.num_results);
        default:

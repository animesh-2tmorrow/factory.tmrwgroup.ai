...
function authenticate(req, res, next) {
    ...
    const provided = req.headers["x-runner-key"] ??
...
app.post("/chat", authenticate, async (req, res) => {
    const { message, history, instructions } = req.body;
    ...
    try {
        ...
        console.log(`[runner] Chat turn complete: model=${result.model} ` +
...
async function start() {
    ...
    console.log(`[runner] Bootstrap state=${bootstrapStatus.state} profile=${bootstrapStatus.profile} project=${bootstrapStatus.projectDir}`);
    if (bootstrapStatus.error) {
        console.error(`[runner] Bootstrap error: ${bootstrapStatus.error}`);
    }
    console.log(`[runner] Agent: ${process.env.AGENT_ID ?? "unknown"} (${process.env.AGENT_NAME ?? "unnamed"})`);
    console.log(`[runner] Model: ${process.env.ANTHROPIC_MODEL ?? process.env.CLOUD_AGENT_MODEL ?? "default"}`);
